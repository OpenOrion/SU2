...
class FSIConfig:
    ...
    def readConfig(self):
        ...
        while 1:
            ...
            # float values
                ...
                (this_param == "RBF_RADIUS")
                or (this_param == "AITKEN_PARAM")
            ...
            # string values
                ...
                (this_param == "CFD_CONFIG_FILE_NAME")
                or (this_param == "CSD_SOLVER")
                or (this_param == "CSD_CONFIG_FILE_NAME")
                or (this_param == "RESTART_SOL")
                or (this_param == "MATCHING_MESH")
                or (this_param == "MESH_INTERP_METHOD")
                or (this_param == "DISP_PRED")
                or (this_param == "AITKEN_RELAX")
    ...
    def applyDefaults(self):
        ...
        if "MAPPING_MODES" not in self._ConfigContent:
            ...
            self.MPIPrint(
                "MAPPING_MODES keyword was not found in the configuration file of the interface, setting to NO",
                False,
            )
        ...
        if "IMPOSED_MOTION" not in self._ConfigContent:
            ...
            self.MPIPrint(
                "IMPOSED_MOTION keyword was not found in the configuration file of the interface, setting to NO",
                False,
            )
        ...
        if self._ConfigContent["IMPOSED_MOTION"] == "YES":
            if (
                ...
                or self._ConfigContent["AITKEN_PARAM"] != 1.0
            ):
                self.MPIPrint(
                    "When imposing motion, the Aitken parameter must be static and equal to 1",
                    True,
                )
        ...
        if self._ConfigContent["RESTART_SOL"] == "YES":
            if self._ConfigContent["TIME_TRESHOLD"] != -1:
                self.MPIPrint(
                    "When restarting a simulation, the time threshold must be -1 for immediate coupling",
                    True,
                )

        if (
            self._ConfigContent["MAPPING_MODES"] == "YES"
            and self._ConfigContent["CSD_SOLVER"] != "NATIVE"
        ):
