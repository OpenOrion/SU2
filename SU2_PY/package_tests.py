...
def io0():
    folder = "test_io0"
    pull = "config_NACA0012.cfg"
    link = "mesh_NACA0012.su2"
...
def level2():
    ...
    with SU2.io.redirect_folder(folder, pull, link):
        ...
        with SU2.io.redirect_folder(folder="JOB_001", link="mesh_NACA0012.su2"):
            ...
            grads = SU2.eval.grad(
                "LIFT", "CONTINUOUS_ADJOINT", config, state
            )  # will not run adjoint
            grads = SU2.eval.grad(
                "DRAG", "CONTINUOUS_ADJOINT", config, state
            )  # will run adjoint
