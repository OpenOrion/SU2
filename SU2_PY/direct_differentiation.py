...
def direct_differentiation(filename, partitions=0, quiet=False, nzones=1):
    # Config
    ...
    if (
        ...
        if config.get("TIME_MARCHING", "NO") == "DUAL_TIME_STEPPING-2ND_ORDER":
            ...
            if not os.path.isfile("DIRECTDIFF/DIRECT/" + restart_filename):
                ...
                if not os.path.isfile(restart_filename):
                    sys.exit(
                        "Error: Restart file <" + restart_filename + "> not found."
                    )
                shutil.copyfile(
                    restart_filename, "DIRECTDIFF/DIRECT/" + restart_filename
                )
